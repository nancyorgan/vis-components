...
// Formerly lib/aggregateBars.test.ts — folded in here when the
// `aggregateBars` re-export shim was removed. Same subject
// (`aggregateStacks`), exercised over a shared multi-region fixture with a
// focus on counting vs. summing and on rows the aggregator must skip.
describe("aggregateStacks counting, summing, and row skipping", () => {
	const rows = [
