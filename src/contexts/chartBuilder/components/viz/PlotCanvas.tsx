...
export const PlotCanvas = () => {
	...
	const dataLabelOverflow = useMemo(() => {
		...
		// Longest RENDERED label across all rows, composed identically to the
		// layer via `buildLabelText`. In multi-field mode that's the full
		// template output (wider than any single field), so the reserved
		// margin tracks what actually draws — otherwise the combined label
		// clips off the right, the very bug the measure-based reserve fixes.
		let longestStr = ""
		for (const row of dataset.rows) {
			const text = buildLabelText(row, value, labelCfg, fallbackField)
			if (text && text.length > longestStr.length) longestStr = text
		}
		if (longestStr === "") return empty
		...
		const xOffset = dataLabels.xOffset ?? 0
		const alignment = dataLabels.alignment ?? "center"
		...
		const rightFraction =
			alignment === "left" ? 1 : alignment === "center" ? 0.5 : 0
		const leftFraction =
		...
		const right = cap(xOffset + labelPx * rightFraction - BASE_MARGIN.right)
		const left = cap(-xOffset + labelPx * leftFraction - BASE_MARGIN.left)
		return { left, right }
	}, [
		dataset,
